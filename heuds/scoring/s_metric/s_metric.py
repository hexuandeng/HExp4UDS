...
from heuds.scoring.s_metric.candidate_mappings import CandidateMappings
from heuds.scoring.s_metric.weight_dict import WeightDict
from heuds.scoring.s_metric import utils
from heuds.scoring.s_metric.repr import Triple, FloatTriple
...
class S(object):
    ...
    def get_best_match(cls, instance1, attribute1, relation1,
        ...
        """
        instance1 = [Triple(x[1], x[0], x[2]) for x in instance1]
        attribute1 = [FloatTriple(x[0], x[1], x[2]) for x in attribute1]
        relation1 = [Triple(x[1], x[0], x[2]) for x in relation1]
        instance2 = [Triple(x[1], x[0], x[2]) for x in instance2]
        attribute2 = [FloatTriple(x[0], x[1], x[2]) for x in attribute2]
        relation2 = [Triple(x[1], x[0], x[2]) for x in relation2]
